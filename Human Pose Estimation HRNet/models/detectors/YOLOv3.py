...
# Cambiar el tamaño de una imagen rectangular a un rectángulo múltiple de 32 píxeles
    shape = img.shape[:2]  # forma actual [alto, ancho]
    if isinstance(new_shape, int):
        ratio = float(new_shape) / max(shape)
    else:
        ratio = max(new_shape) / max(shape)  # Relación = nuevo / viejo
    new_unpad = (int(round(shape[1] * ratio)), int(round(shape[0] * ratio)))

    if mode is 'auto':  # rectángulo mínimo
        dw = np.mod(new_shape - new_unpad[0], 32) / 2  # ancho
        dh = np.mod(new_shape - new_unpad[1], 32) / 2  # altura
    elif mode is 'square':  # cuadrado
        dw = (new_shape - new_unpad[0]) / 2  # ancho
        dh = (new_shape - new_unpad[1]) / 2  # altura
    else:
        raise NotImplementedError

    top, bottom = int(round(dh - 0.1)), int(round(dh + 0.1))
    left, right = int(round(dw - 0.1)), int(round(dw + 0.1))
    img = cv2.resize(img, new_unpad, interpolation=cv2.INTER_LINEAR)  # redimensionado, sin borde
    img = cv2.copyMakeBorder(img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=color)  # padded square
    return img, ratio, dw, dh

def scale_coords(coords, from_image_shape, to_image_shape):
    gain = max(from_image_shape) / max(to_image_shape)  #Ganancia = viejo / nuevo
    coords[:, [0, 2]] -= (from_image_shape[1] - to_image_shape[1] * gain) / 2  # x 
    coords[:, [1, 3]] -= (from_image_shape[0] - to_image_shape[0] * gain) / 2  # y 
    coords[:, :4] /= gain
...
class YOLOv3:
    def __init__(self,
        ...
        if weights_path.endswith(".weights"):
            #Cargar pesos de darknet
            self.model.load_darknet_weights(weights_path)
        else:
            #Carga de pesos de control
            self.model.load_state_dict(torch.load(weights_path))

        self.model.eval()  #Establecer en modo de evaluación

        self.classes_file = load_classes(class_path)  #Extrae etiquetas de clase del archivo
